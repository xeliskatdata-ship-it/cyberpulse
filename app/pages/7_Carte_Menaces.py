...
# Supprime le canvas ECG si present (la carte a son propre rendu)
components.html("""
<script>
(function() {
...
# ==============================================================
# PROXIMITE PAYS / MOT-CLE CYBER (spaCy)
# ==============================================================
_CYBER_VICTIM_WORDS = {
...
# ==============================================================
# CLASSIFICATION / KEYWORDS / SEVERITY
# ==============================================================
_PATTERNS = [
...
def _compute_confidence(gpe_count, has_org, regex_match, spacy_org_match,
    ...
    # Axe 3 — Signal cyber (0-15)
    if strong_kw_in_300:
        score += 15
    elif has_strong_kw:
        score += 6

    # Penalites (jusqu'a -28)
    if source_fallback:
        score -= 20
    if multiple_victims:
        score -= 8
...
def extract_target(title, desc, source=""):
    ...
    # Etape 1 : patterns de contexte directs
    ...
    # Etape 2b : scan description (premiers 1000 chars)
    if target_country is None:
    ...
    # Etape 3 : spaCy NER avec scoring de proximite
    nlp = _get_nlp(source)
    if nlp is not None:
        doc = nlp(text[:1500])
        gpe_candidates = []
        ...
        # Meilleur GPE par score de proximite
        if target_country is None and gpe_candidates:
            gpe_candidates.sort(key=lambda x: -x[2])
            target_country, target_lat, target_lon = gpe_candidates[0][1]

        # Validation croisee regex/spaCy pour l'org victime
...
# LOAD DATA
# ==============================================================
df_raw = get_stg_articles(limit=1000)
try:
    df_cve = get_mart_k6()
    n_cve = len(df_cve)
except Exception:
...
def _build_events(_df):
    ...
    # Deduplication sur titre normalise
    seen = set()
    dedup = []
    for ev in evts:
...
events = _build_events(df_raw)

# Stats — Counter au lieu de boucle manuelle
n_events = len(events)
n_countries = len({e["country"] for e in events})
n_crit = sum(1 for e in events if e["severity"] == "critical")
cat_counts = Counter(e["cat"] for e in events)
