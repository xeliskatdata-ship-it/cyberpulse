# 1_kpi1_Articles.py -- KPI 1 : Articles collectes par jour / par source
# Design : fond bokeh, ECG, small multiples, heatmap
...
(function() {
  ...
  function startECG(){var old=p.getElementById('ecg-bg');if(old)old.parentNode.removeChild(old);
    ...
    var dots=[];for(var i=0;i<50;i++)dots.push({x:Math.random()*cv.width,y:Math.random()*cv.height,
      r:Math.random()*1.5+0.3,phase:Math.random()*Math.PI*2,speed:Math.random()*0.008+0.004,
    ...
    function draw(){if(!p.getElementById('ecg-bg')||!alive)return;
      ...
      if(history.length>1){for(var k=1;k<history.length;k++){
        var prog=k/history.length,alpha=prog*0.85,isSpike=Math.abs(history[k].y-H/2)>H*0.08;
...
SOURCE_GROUPS = {
    ...
    "API REST": ["NewsAPI"],
    "Cyber généraliste": [
        "The Hacker News","BleepingComputer","CISA Alerts","Krebs on Security",
        "Dark Reading","SecurityWeek","Cyber Scoop","Threatpost","Schneier on Security",
        "The Record","Infosecurity Magazine","Helpnet Security","Graham Cluley",
        "Zataz","ANSSI","CERT-EU","French Breaches",
    ],
    "Cyber spécialisé": [
        "Malwarebytes Labs","Naked Security","We Live Security",
        "Trend Micro","Recorded Future Blog","Cybereason",
    ],
    "OSINT": ["Bellingcat"],
    "Threat Intelligence": [
        "SANS ISC","Mandiant Blog","CrowdStrike Blog",
...
# ── Section titre helper ──────────────────────────────────────────────────────
def _section(title):
    st.markdown(
        f"<div style='text-align:center;font-family:Roboto Mono,monospace;font-size:1.4rem;"
        f"letter-spacing:.1em;text-transform:uppercase;color:#3b82f6;"
        f"border-bottom:1px solid #3b82f6;padding-bottom:8px;width:fit-content;"
        f"margin:28px auto 16px'>{title}</div>",
        unsafe_allow_html=True,
    )
...
try:
    ...
    with col_b:
        ...
        st.markdown(
            ...
            f'Dernière mise à jour : le {now.strftime("%d/%m/%Y")} à {now.strftime("%H:%M:%S")}'
            f' · {len(df):,} articles</div>',
            unsafe_allow_html=True,
        )
...
sources_dispo = sorted(df["source"].dropna().unique())
sources = (
    [s for s in SOURCE_GROUPS[groupe_sel] if s in sources_dispo]
    if SOURCE_GROUPS[groupe_sel]
    else list(sources_dispo)
)
...
agg_total = agg.groupby("date")["nb_articles"].sum().reset_index(name="total")
...
transition:border-color 0.2s,transform 0.2s,box-shadow 0.2s;cursor:default;backdrop-filter:blur(8px)}}
...
<div class="grid">
  <div class="card"><div class="val" id="v1">0</div><div class="lbl">Articles collectés</div><div class="sub">{n_days} jours</div></div>
  <div class="card"><div class="val" id="v2">0</div><div class="lbl">Sources actives</div><div class="sub">sur {len(sources)} sélectionnées</div></div>
  <div class="card"><div class="val" style="font-size:1.8rem" id="v3">{top_src_short}</div><div class="lbl">Source la + active</div><div class="sub">{top_total} articles</div></div>
  <div class="card"><div class="val" id="v4">0</div><div class="lbl">Moy. articles/jour</div><div class="sub">toutes sources</div></div>
</div>
<script>
function animCount(id,target,duration,isFloat){{var el=document.getElementById(id);if(!el||isNaN(target))return;
var step=target/(duration/16),current=0;var timer=setInterval(function(){{current+=step;
if(current>=target){{current=target;clearInterval(timer);}}
el.textContent=isFloat?current.toFixed(1).replace('.',','):Math.floor(current).toLocaleString('fr-FR');}},16);}}
animCount('v1',{total_global},1200,false);animCount('v2',{nb_src_actives},800,false);animCount('v4',{moy_jour},1000,true);
</script>
""", height=160)

# ── Small Multiples ───────────────────────────────────────────────────────────
_section("Small multiples — activité par source")
st.markdown(
    "<div style='text-align:center;color:#94a3b8;font-size:1.1rem;margin-bottom:20px'>"
    "Chaque mini-graphique représente une source. "
    "La courbe montre le volume d'articles collectés par jour.</div>",
    unsafe_allow_html=True,
)
...
if sources_actives:
    ...
    for idx, src in enumerate(sources_actives):
        ...
        fig_sm.add_trace(
            go.Bar(
                x=src_data["date"], y=src_data["nb_articles"],
                name=src, marker_color=color, opacity=0.85,
    ...
    for i in range(1, n_rows * n_cols + 1):
        ...
        fig_sm.update_xaxes(showticklabels=False, showgrid=False, zeroline=False, row=r, col=c)
        fig_sm.update_yaxes(showgrid=True, gridcolor="rgba(30,42,66,0.5)",
                            tickfont=dict(size=9, color="#64748b"), zeroline=False, row=r, col=c)

    fig_sm.update_layout(**PLOTLY_BASE, height=max(300, n_rows * 130), margin=dict(l=20, r=20, t=40, b=20))
...
# ── Heatmap ───────────────────────────────────────────────────────────────────
_section("Intensité par source & par jour")
st.markdown(
    "<div style='text-align:center;color:#94a3b8;font-size:1.1rem;margin-bottom:20px'>"
    "Plus la couleur est claire, plus la source a édité d'articles ce jour-là.</div>",
    unsafe_allow_html=True,
)

heatmap_data = agg.pivot_table(index="source", columns="date", values="nb_articles", aggfunc="sum", fill_value=0)
